...
class EnrichmentEngine {
  async enrich(rawTexts, onProgress) {
    const allTx = [];

    // Step 1: Parse all texts
    onProgress?.('Parsing your files...');
    for (const text of rawTexts) {
      let txs = this.parseCSV(text);
      // If too few transactions parsed from a large text, skip — PDF/unstructured
      // extraction is handled separately via an API route
      allTx.push(...txs);
    }
    ...
    if (_datedAll.length >= 2) {
      const _latest = new Date(Math.max(..._datedAll.map(tx => tx.date.getTime())));
      const _cutoff = new Date(_latest);
      _cutoff.setMonth(_cutoff.getMonth() - 4);
      const _keep = allTx.filter(tx => !tx.date || tx.date >= _cutoff);
      if (_keep.length >= 5) {
        allTx.length = 0;
        allTx.push(..._keep);
      }
    ...
    onProgress?.('Identifying merchants...');
    let enriched = allTx.map(tx => this.enrichTransaction(tx));

    // Step 3: Local categorisation only (Claude verification available via separate API route)
    onProgress?.('Categorising data...');
    enriched = this.batchVerify(enriched);

    // Step 4: Build profile
    onProgress?.('Building your profile...');
    ...
    onProgress?.('Analysing your decisions...');
    ...
    return {
      profile, archetype, traits, strengths, blindSpots, peerComparison,
      insights, potentialSavings, decisionScore, decisionStack,
      behavioralPatterns, compoundCosts, playbook,
      subscriptions: profile.subscriptions, enrichedTransactions: enriched
    };
  }

  // Local-only batch verification pass-through — returns transactions as-is after local enrichment.
  // Claude AI verification is available via a separate API route and can be layered on top.
  batchVerify(enrichedTxs) {
    return enrichedTxs;
  }
  ...
  parseDate(s) {
    ...
    if (m) {
      ...
      return new Date(m[3], months[m[2].toLowerCase().slice(0, 3)], m[1]);
    }
    const d = new Date(s);
  ...
  norm(d) {
    return d.toLowerCase().replace(/[^\w\s]/g, ' ').replace(/\d{4,}/g, '').replace(/\s+/g, ' ').trim();
  }
  ...
  enrichTransaction(tx) {
    ...
    for (const [pattern, data] of this._sortedPatterns) {
      if (data._wordBound) {
        const p = pattern.trim();
        const idx = n.indexOf(p);
        if (idx >= 0 && (idx === 0 || n[idx - 1] === ' ') && (idx + p.length >= n.length || n[idx + p.length] === ' ')) {
          lookup = data; confidence = 'high'; break;
        }
    ...
    return {
      ...
      category: (isLargeCredit || isCompanyCredit) ? 'Income' : lookup.category,
      subcategory: lookup.subcategory || '',
      isSubscription: lookup.isSubscription,
      isIncome: isIncomeCategory || isLargeCredit || isCompanyCredit,
      isRefund,
      isSavings,
      isTransfer: lookup.category === 'Transfers' || isSavings,
      isDebt: lookup.category === 'Debt Payments',
      confidence
    };
  }
  ...
  detectRecurring(txs) {
    const byM = {};
    txs.forEach(tx => {
      if (!tx.merchant || tx.isIncome || tx.isTransfer || tx.isRefund) return;
      if (this._isLikelyPersonName(tx.merchant)) return;
      if (!byM[tx.merchant]) byM[tx.merchant] = [];
      byM[tx.merchant].push(tx);
    });
    const recurring = [];
    for (const [merchant, list] of Object.entries(byM)) {
      ...
      for (let i = 1; i < list.length; i++) {
        if (list[i].date && list[i - 1].date) {
          const days = Math.round((list[i].date - list[i - 1].date) / (1000 * 60 * 60 * 24));
      ...
      if (Math.sqrt(variance) < avg * 0.4 && avg > 5) {
        ...
        recurring.push({
          merchant, frequency: freq, averageAmount: Math.round(avgAmt * 100) / 100,
          category: list[0].category, subcategory: list[0].subcategory,
          isSubscription: list[0].isSubscription, isDebt: list[0].isDebt
        });
  ...
  buildProfile(txs, recurring) {
    ...
    spending.forEach(tx => {
      ...
      if (!byCat[k]) byCat[k] = { total: 0, count: 0, txs: [] };
      byCat[k].total += Math.abs(tx.amount);
      byCat[k].count++;
      byCat[k].txs.push(tx);
    });
    ...
    let months = 1;
    if (dated.length >= 2) {
      const dates = dated.map(tx => tx.date.getTime());
      const earliest = Math.min(...dates);
      const latest = Math.max(...dates);
      months = Math.max(1, (latest - earliest) / (1000 * 60 * 60 * 24 * 30.44)) || 1;
    }
    ...
    spending.forEach(tx => {
      ...
      if (this._isLikelyPersonName(tx.merchant)) return;
      if (!byMerch[tx.merchant]) byMerch[tx.merchant] = { total: 0, count: 0 };
      byMerch[tx.merchant].total += Math.abs(tx.amount);
      byMerch[tx.merchant].count++;
    });
    ...
    income.forEach(tx => {
      ...
      incomeBySource[key].total += tx.amount;
      incomeBySource[key].count++;
      incomeBySource[key].amounts.push(tx.amount);
      if (tx.date) incomeBySource[key].dates.push(tx.date);
    });
    ...
    const credByKey = {};
    largeCreds.forEach(tx => {
      const key = tx.descNorm?.split(' ').filter(w => w.length > 2).slice(0, 2).join(' ') || 'unknown';
      if (!credByKey[key]) credByKey[key] = { total: 0, count: 0, dates: [], merchant: tx.merchant || key };
      credByKey[key].total += tx.amount;
      credByKey[key].count++;
      credByKey[key].dates.push(tx.date);
    });
    for (const [key, src] of Object.entries(credByKey)) {
      if (src.count >= 2 && !incomeBySource[key]) {
        src.dates.sort((a, b) => a - b);
        const ints = [];
        for (let i = 1; i < src.dates.length; i++) {
          const d = Math.round((src.dates[i] - src.dates[i - 1]) / (1000 * 60 * 60 * 24));
          if (d > 0) ints.push(d);
        }
        if (ints.length > 0) {
          ...
          if (avg > 20 && avg < 45) {
            incomeBySource[key] = {
              ...src,
              amounts: largeCreds.filter(tx => (tx.descNorm?.split(' ').filter(w => w.length > 2).slice(0, 2).join(' ') || 'unknown') === key).map(tx => tx.amount),
              isSalary: false
            };
          }
        }
    ...
    const _calcFreq = (dates) => {
      if (dates.length < 2) return 'irregular';
      dates.sort((a, b) => a - b);
      const ints = [];
      for (let i = 1; i < dates.length; i++) {
        const d = Math.round((dates[i] - dates[i - 1]) / (1000 * 60 * 60 * 24));
        if (d > 0) ints.push(d);
      }
      if (!ints.length) return 'irregular';
      const avg = ints.reduce((a, b) => a + b, 0) / ints.length;
      return avg < 10 ? 'weekly' : avg < 18 ? 'fortnightly' : avg < 45 ? 'monthly' : 'irregular';
    };
    const incomeSources = Object.values(incomeBySource).map(src => {
      ...
      return {
        source: src.merchant, frequency, avgAmount, total: Math.round(src.total),
        count: src.count, monthly: mo(src.total), isPrimary: false, isSalary: src.isSalary || false
      };
    }).sort((a, b) => b.total - a.total);
    // Mark primary income source — salary first, then largest regular source
    if (incomeSources.length > 0) {
      const primary = incomeSources.find(s => s.isSalary) || incomeSources.find(s => s.frequency === 'monthly' && s.monthly > 500) || incomeSources[0];
      if (primary) {
        primary.isPrimary = true;
        primary.source = primary.isSalary ? 'Primary Income (Salary)' : primary.source + ' (Primary)';
      }
    }
    ...
    for (const [cat, data] of Object.entries(byCat)) {
      ...
      if (nonDiscretionaryCats.includes(cat) || (cat === 'Subscriptions')) {
        nonDiscretionary.total += monthly;
        nonDiscretionary.items.push({ category: cat, monthly, txs: data.txs || [] });
      } else if (cat !== 'Other' && cat !== 'Transfers' && cat !== 'Savings' && cat !== 'Refund') {
        discretionary.total += monthly;
        discretionary.items.push({ category: cat, monthly, txs: data.txs || [] });
      }
    }
    // Add subscription + debt from recurring (more accurate than category totals)
    const subMo = Math.round(subTotal);
    const debtMo = Math.round(debtTotal);
    ...
    return {
      ...
      monthly: {
        income: mo(totalIncome), spending: mo(totalSpending), surplus: mo(surplus),
        subscriptions: Math.round(subTotal), debtPayments: Math.round(debtTotal),
        foodDelivery: mo(byCat['Food Delivery']?.total || 0),
        shopping: mo(byCat['Shopping']?.total || 0),
        entertainment: mo(byCat['Entertainment']?.total || 0),
        groceries: mo(byCat['Groceries']?.total || 0),
        transport: mo(byCat['Transport']?.total || 0),
        eatingOut: mo(byCat['Eating Out']?.total || 0),
        health: mo(byCat['Health']?.total || 0),
        travel: mo(byCat['Travel']?.total || 0),
        charity: mo(byCat['Charity']?.total || 0),
        education: mo(byCat['Education']?.total || 0)
      },
      metrics: {
        savingsRate: totalIncome > 0 ? Math.round((surplus / totalIncome) * 100) : (totalSpending > 0 ? -100 : 0),
        subscriptionCount: subs.length,
        debtAccountCount: debtPay.length,
  ...
  determineArchetype(p) {
    ...
    for (const k of order) {
      try { if (ARCHETYPES[k].triggers(p)) return { key: k, ...ARCHETYPES[k] }; } catch {}
    }
  ...
  calcPeerComparison(p) {
    ...
    if (mt.subscriptionCount > 0) {
      const d = Math.round(((mt.subscriptionCount - UK_BENCHMARKS.subscriptions.count) / UK_BENCHMARKS.subscriptions.count) * 100);
      comps.push({ label: 'Subscriptions', value: mt.subscriptionCount, unit: '', diff: d, isHigher: d > 0 });
    }
    if (m.foodDelivery > 0) {
      const d = Math.round(((m.foodDelivery - UK_BENCHMARKS.foodDelivery) / UK_BENCHMARKS.foodDelivery) * 100);
      comps.push({ label: 'Food Delivery', value: m.foodDelivery, unit: '\u00a3/mo', diff: d, isHigher: d > 0 });
    }
  ...
  genInsights(p) {
    ...
    if (p.subscriptions.length > 0) {
      ins.push({ type: 'subscriptions', text: 'You have ' + p.subscriptions.length + ' active subscriptions totalling \u00a3' + m.subscriptions + '/mo (\u00a3' + (m.subscriptions * 12) + '/yr): ' + p.subscriptions.slice(0, 4).map(s => s.merchant + ' \u00a3' + s.averageAmount).join(', ') + (p.subscriptions.length > 4 ? ' and ' + (p.subscriptions.length - 4) + ' more' : '') + '.' });
    }
    if (p.debtPayments.length > 0) {
      ins.push({ type: 'debt', text: 'You have ' + p.debtPayments.length + ' debt payment' + (p.debtPayments.length > 1 ? 's' : '') + ' totalling \u00a3' + m.debtPayments + '/mo (\u00a3' + (m.debtPayments * 12) + '/yr): ' + p.debtPayments.slice(0, 3).map(d => d.merchant).join(', ') + '.' + (m.income > 0 ? ' That is ' + Math.round((m.debtPayments / m.income) * 100) + '% of your income.' : '') });
    }
    if (m.foodDelivery > 50) {
      ins.push({ type: 'delivery', text: 'Food delivery is costing you \u00a3' + m.foodDelivery + '/month (\u00a3' + (m.foodDelivery * 12) + '/yr). Cooking at home for just 2-3 of those orders per week would save roughly \u00a3' + Math.round(m.foodDelivery * 0.4 * 12) + '/year.' });
    }
    if ((m.eatingOut || 0) > 80) {
      ins.push({ type: 'eatingout', text: 'You are spending \u00a3' + (m.eatingOut || 0) + '/month eating out and on coffee (\u00a3' + ((m.eatingOut || 0) * 12) + '/yr). Small daily purchases add up faster than most people realise.' });
    }
    if ((m.groceries || 0) > 0 && m.foodDelivery > 0) {
      const ratio = m.foodDelivery / (m.groceries || 1);
      if (ratio > 0.5) {
        ins.push({ type: 'food', text: 'Your food delivery spend (\u00a3' + m.foodDelivery + '/mo) is ' + Math.round(ratio * 100) + '% of your grocery spend (\u00a3' + (m.groceries || 0) + '/mo). A healthier ratio would be under 25%.' });
      }
    }
    if ((m.shopping || 0) > 100) {
      ins.push({ type: 'shopping', text: 'Shopping accounts for \u00a3' + (m.shopping || 0) + '/month (\u00a3' + ((m.shopping || 0) * 12) + '/yr). ' + (p.topMerchants?.find(m => ['Amazon', 'ASOS', 'Zara', 'IKEA', 'H&M', 'Primark', 'Shein', 'Boohoo'].includes(m.merchant)) ? 'Your top retailer is ' + p.topMerchants.find(m => ['Amazon', 'ASOS', 'Zara', 'IKEA', 'H&M', 'Primark', 'Shein', 'Boohoo'].includes(m.merchant)).merchant + '.' : '') });
    }
    if (p.spendingTrend) {
      const t = p.spendingTrend;
      if (t.direction === 'up') {
        ins.push({ type: 'trend', text: 'Your spending in the last 30 days (\u00a3' + t.recentMonthTotal + ') is ' + t.pctChange + '% higher than your monthly average (\u00a3' + t.averageMonthly + '). Top categories this month: ' + t.topRecentCategories.map(c => c.name + ' \u00a3' + c.total).join(', ') + '.' });
      } else if (t.direction === 'down') {
        ins.push({ type: 'trend', text: 'Good news \u2014 your spending in the last 30 days (\u00a3' + t.recentMonthTotal + ') is ' + Math.abs(t.pctChange) + '% lower than your monthly average (\u00a3' + t.averageMonthly + '). You are trending in the right direction.' });
      }
    }
    return ins;
  }
  ...
  calcSavings(p) {
    ...
    const add = (cat, monthly, pct, tip) => {
      const yearly = monthly * 12;
      const saving = Math.round(yearly * pct);
      if (saving > 0) items.push({ category: cat, monthlySpend: monthly, yearlySpend: yearly, savingAmount: saving, savingPct: Math.round(pct * 100), tip });
    };
  ...
  detectBehavioralPatterns(txs) {
    ...
    if (income.length >= 2) {
      ...
      if (postPayday.length > 0 && restDays.length > 0) {
        ...
        if (ratio > 1.5) {
          const premium = Math.round((postAvgDaily - restAvgDaily) * 3);
          patterns.push({ pattern: 'payday_splurge', text: 'You spend ' + ratio + 'x more in the 3 days after payday', detail: 'That is roughly \u00a3' + premium + ' above your daily average each pay cycle', severity: ratio > 2 ? 'high' : 'medium' });
        }
      }
    }
    ...
    if (weekend.length > 3 && weekday.length > 5) {
      ...
      if (ratio > 1.3) {
        patterns.push({ pattern: 'weekend_premium', text: 'Your weekend spending is ' + ratio + 'x your weekday average', detail: '\u00a3' + Math.round(wkndDaily) + '/day on weekends vs \u00a3' + Math.round(wkdyDaily) + '/day on weekdays', severity: ratio > 1.8 ? 'high' : 'medium' });
      }
    }
    ...
    const byMerch = {};
    spending.forEach(tx => {
      if (!tx.merchant || tx.isSubscription || tx.category === 'Transfers' || tx.category === 'Debt Payments') return;
      if (this._isLikelyPersonName(tx.merchant)) return;
      if (!byMerch[tx.merchant]) byMerch[tx.merchant] = { count: 0, total: 0, category: tx.category };
      byMerch[tx.merchant].count++;
      byMerch[tx.merchant].total += Math.abs(tx.amount);
    });
    const bpMonths = Math.max(1, txs.filter(tx => tx.date).length >= 2 ? ((Math.max(...txs.filter(tx => tx.date).map(tx => tx.date.getTime())) - Math.min(...txs.filter(tx => tx.date).map(tx => tx.date.getTime()))) / (1000 * 60 * 60 * 24 * 30.44)) : 1);
    for (const [merchant, data] of Object.entries(byMerch)) {
      if (data.count >= 8) {
        const monthly = Math.round(data.total / bpMonths);
        ...
        if (isEssential) {
          ...
          if (monthly > 100) {
            patterns.push({ pattern: 'essential_review', text: merchant + ': \u00a3' + monthly + '/mo on ' + data.category.toLowerCase() + ' (essential)', detail: 'This is a necessary expense but worth reviewing if the amount seems high. Consider if there are more cost-effective alternatives.', severity: 'low', merchant, monthlyAmount: monthly, isEssential: true });
          }
        } else {
    ...
    if (income.length >= 2) {
      ...
      if (lateMonth.length > 2 && midMonth.length > 5) {
        ...
        if (dropPct > 30) {
          patterns.push({ pattern: 'late_month_squeeze', text: 'Your spending drops ' + dropPct + '% in the last 5 days before payday', detail: 'This suggests cash flow pressure at end of month', severity: dropPct > 50 ? 'high' : 'medium' });
        }
      }
    }
    ...
    if (clusters.length >= 2) {
      const avgCluster = Math.round(clusters.reduce((s, c) => s + c.reduce((t, tx) => t + Math.abs(tx.amount), 0), 0) / clusters.length);
      patterns.push({ pattern: 'impulse_cluster', text: clusters.length + ' shopping spree days detected', detail: 'You average \u00a3' + avgCluster + ' on days with 3+ shopping transactions', severity: avgCluster > 100 ? 'high' : 'medium' });
    }
  ...
  calcDecisionScore(profile) {
    ...
    if (totalScore >= 80) verdict = 'Strong financial discipline \u2014 you are making your money work for you';
      ...
      verdict = 'Room to improve \u2014 your biggest lever is ' + weakest.label.toLowerCase();
    } else {
      const leaked = Math.round(totalSpend * 0.2);
      verdict = '\u00a3' + leaked + '/mo is going to habits you may not be actively choosing';
    }
  ...
  genDecisionStack(profile, recurring, behavioralPatterns) {
    ...
    if (subs.length > 3) {
      const cheapest = subs.filter(s => s.averageAmount < 15).slice(0, 3);
      if (cheapest.length > 0) {
        const total = Math.round(cheapest.reduce((s, r) => s + r.averageAmount, 0));
        moves.push({
          action: 'Review ' + cheapest.length + ' low-cost subscriptions: ' + cheapest.map(s => s.merchant + ' (\u00a3' + s.averageAmount + ')').join(', '),
          annualImpact: total * 12, effort: 'low',
          unlocks: 'Quick wins \u2014 often forgotten services that add up',
          type: 'subscription',
          details: {
            strategy: 'Subscription audit',
            items: cheapest.map(s => ({ name: s.merchant, amount: s.averageAmount, frequency: 'monthly' })),
            monthlyAmount: total,
            reasoning: 'These ' + cheapest.length + ' subscriptions total \u00a3' + total + '/mo. Low-cost subscriptions often go unnoticed but compound over time. Cancelling unused ones frees up \u00a3' + (total * 12) + '/yr with zero lifestyle impact.',
            steps: ['Review each subscription for recent usage', 'Cancel any you haven\u2019t used in 30+ days', 'Set a calendar reminder to audit subscriptions quarterly']
          }
        });
      }
    }
    ...
    if (m.foodDelivery > 40) {
      const deliveryMerchants = profile.topMerchants?.filter(tm => ['Deliveroo', 'Uber Eats', 'Just Eat'].includes(tm.merchant)) || [];
      const totalOrders = deliveryMerchants.reduce((s, tm) => s + tm.count, 0) / Math.max(1, profile.monthSpan) || 4;
      const avgOrder = m.foodDelivery / totalOrders;
      const cutOrders = Math.ceil(m.foodDelivery * 0.3 / avgOrder);
      const monthlySave = Math.round(m.foodDelivery * 0.3);
      moves.push({
        action: 'Swap ' + cutOrders + ' delivery orders per month for home cooking',
        annualImpact: Math.round(monthlySave * 12), effort: 'medium',
        unlocks: m.groceries > 0 ? 'Your grocery spend shows you already cook \u2014 just shift the balance' : 'Redirects \u00a3' + monthlySave + '/mo to other goals',
        type: 'convenience',
        details: {
          strategy: 'Delivery-to-cooking swap',
          items: deliveryMerchants.map(tm => ({ name: tm.merchant, amount: Math.round(tm.total / profile.monthSpan), frequency: 'monthly' })),
          monthlyAmount: m.foodDelivery,
          targetReduction: monthlySave,
          reasoning: 'You\u2019re spending \u00a3' + m.foodDelivery + '/mo on delivery across ' + deliveryMerchants.length + ' service' + (deliveryMerchants.length > 1 ? 's' : '') + '. The average order is \u00a3' + Math.round(avgOrder) + '. Swapping just ' + cutOrders + ' orders/mo for home cooking saves \u00a3' + monthlySave + '/mo (\u00a3' + (monthlySave * 12) + '/yr) while still enjoying delivery ' + (Math.round(totalOrders) - cutOrders) + 'x/mo.' + (m.groceries > 0 ? ' Your existing grocery spend of \u00a3' + m.groceries + '/mo shows you already cook \u2014 this just shifts the balance.' : ''),
          steps: ['Identify your most frequent delivery days', 'Prep easy meals for those specific days', 'Keep delivery for social occasions or genuine treats', 'Track progress weekly for the first month']
        }
      });
    }
    ...
    if (debts.length >= 2) {
      const sortedDebts = [...debts].sort((a, b) => a.averageAmount - b.averageAmount);
      const smallest = sortedDebts[0];
      const totalDebtMo = Math.round(debts.reduce((s, d) => s + d.averageAmount, 0));
      const secondDebt = sortedDebts.length > 1 ? sortedDebts[1] : null;
      moves.push({
        action: 'Focus on clearing ' + smallest.merchant + ' first (\u00a3' + smallest.averageAmount + '/mo)',
        annualImpact: Math.round(smallest.averageAmount * 12), effort: 'medium',
        unlocks: 'Once cleared, redirect \u00a3' + smallest.averageAmount + '/mo to the next debt or savings',
        type: 'debt',
        details: {
          strategy: 'Debt Snowball Method',
          items: sortedDebts.map(d => ({ name: d.merchant, amount: d.averageAmount, frequency: 'monthly' })),
          monthlyAmount: totalDebtMo,
          targetDebt: smallest.merchant,
          targetAmount: smallest.averageAmount,
          reasoning: 'You have ' + debts.length + ' active debts totalling \u00a3' + totalDebtMo + '/mo. The snowball method targets your smallest payment first (\u00a3' + smallest.averageAmount + '/mo to ' + smallest.merchant + '). Once cleared, that \u00a3' + smallest.averageAmount + ' rolls into ' + (secondDebt ? secondDebt.merchant + ' (\u00a3' + secondDebt.averageAmount + '/mo)' : 'your next debt') + ', accelerating payoff. This creates momentum and frees up cashflow progressively.',
          steps: ['Continue minimum payments on all debts', 'Put any extra funds toward ' + smallest.merchant + ' (\u00a3' + smallest.averageAmount + '/mo)', 'Once ' + smallest.merchant + ' is cleared, add its payment to ' + (secondDebt ? secondDebt.merchant : 'the next debt'), 'Repeat until all debts are cleared'],
          effect: 'Clearing ' + smallest.merchant + ' first frees \u00a3' + smallest.averageAmount + '/mo. Over 12 months, that\u2019s \u00a3' + (smallest.averageAmount * 12) + ' redirected to accelerate remaining debt payoff or build savings.'
        }
      });
    }
    ...
    if (hidden.length > 0) {
      const top = hidden[0];
      const save30 = Math.round(top.monthlyAmount * 0.3);
      moves.push({
        action: 'Your ' + top.merchant + ' habit is a hidden \u00a3' + top.monthlyAmount + '/mo subscription',
        annualImpact: Math.round(save30 * 12), effort: 'medium',
        unlocks: 'Reducing by 30% saves \u00a3' + save30 + '/mo without eliminating it',
        type: 'behavioral',
        details: {
          strategy: 'Habitual spending reduction',
          items: [{ name: top.merchant, amount: top.monthlyAmount, frequency: 'monthly' }],
          monthlyAmount: top.monthlyAmount,
          targetReduction: save30,
          reasoning: 'Your ' + top.merchant + ' spending of \u00a3' + top.monthlyAmount + '/mo behaves like a subscription \u2014 consistent, recurring, and often automatic. A 30% reduction saves \u00a3' + save30 + '/mo (\u00a3' + (save30 * 12) + '/yr) while keeping the habit at a sustainable level.',
          steps: ['Set a weekly budget of \u00a3' + Math.round(top.monthlyAmount * 0.7 / 4) + ' for ' + top.merchant, 'Track each purchase for 2 weeks to build awareness', 'Replace 1-2 visits per week with a free or cheaper alternative', 'Review progress after 30 days']
        }
      });
    }
    ...
    if (splurge) {
      const premium = parseInt(splurge.detail.match(/\d+/)?.[0] || '0');
      if (premium > 30) {
        const save50 = Math.round(premium * 0.5);
        moves.push({
          action: 'Delay non-essential purchases 48 hours after payday',
          annualImpact: Math.round(save50 * 12), effort: 'low',
          unlocks: 'Your post-payday spike adds \u00a3' + premium + '/mo \u2014 a short delay cuts impulse buys in half',
          type: 'behavioral',
          details: {
            strategy: '48-hour payday rule',
            items: [{ name: 'Post-payday premium spending', amount: premium, frequency: 'monthly' }],
            monthlyAmount: premium,
            targetReduction: save50,
            reasoning: 'Your spending spikes significantly in the days after payday, costing an extra \u00a3' + premium + '/mo compared to your baseline. Research shows a simple 48-hour delay eliminates roughly half of impulse purchases. This saves \u00a3' + save50 + '/mo (\u00a3' + (save50 * 12) + '/yr) with minimal effort.',
            steps: ['When payday hits, move discretionary funds to a separate pot', 'Wait 48 hours before any non-essential purchase over \u00a320', 'After 48 hours, buy only items you still want', 'Track how many purchases you skip \u2014 most won\u2019t feel like sacrifices']
          }
        });
      }
    }
    ...
    if ((m.eatingOut || 0) > 80) {
      const coffeeShops = profile.topMerchants?.filter(tm => ['Starbucks', 'Costa', 'Pret', 'Greggs', 'Caffe Nero'].includes(tm.merchant)) || [];
      const coffeeMonthly = coffeeShops.reduce((s, tm) => s + Math.round(tm.total / profile.monthSpan), 0);
      if (coffeeMonthly > 20) {
        const coffeeSave = Math.round(coffeeMonthly * 0.5);
        moves.push({
          action: 'Your coffee habit: \u00a3' + coffeeMonthly + '/mo across ' + coffeeShops.length + ' shop' + (coffeeShops.length > 1 ? 's' : ''),
          annualImpact: Math.round(coffeeSave * 12), effort: 'low',
          unlocks: 'Making coffee 3x/week instead saves \u00a3' + coffeeSave + '/mo',
          type: 'convenience',
          details: {
            strategy: 'Coffee spend optimisation',
            items: coffeeShops.map(tm => ({ name: tm.merchant, amount: Math.round(tm.total / profile.monthSpan), frequency: 'monthly' })),
            monthlyAmount: coffeeMonthly,
            targetReduction: coffeeSave,
            reasoning: 'You\u2019re spending \u00a3' + coffeeMonthly + '/mo across ' + coffeeShops.length + ' coffee shop' + (coffeeShops.length > 1 ? 's' : '') + '. Making coffee at home 3 days per week saves roughly 50%, or \u00a3' + coffeeSave + '/mo (\u00a3' + (coffeeSave * 12) + '/yr). You still get to enjoy coffee out on the days that matter.',
            steps: ['Get a good travel mug and coffee you enjoy at home', 'Choose 3 specific days per week to make coffee at home', 'Keep your coffee-out days for social or treat occasions', 'A \u00a320 bag of quality beans makes ~30 cups vs \u00a33-4 each out']
          }
        });
      }
    }
    ...
    if ((m.shopping || 0) > 120) {
      const impulsePattern = behavioralPatterns.find(p => p.pattern === 'impulse_cluster');
      const shopSave = Math.round((m.shopping || 0) * 0.2);
      moves.push({
        action: 'Apply a 24-hour rule to purchases over \u00a330',
        annualImpact: Math.round(shopSave * 12), effort: 'low',
        unlocks: impulsePattern ? impulsePattern.detail : 'Reduces impulse purchases by roughly 20%',
        type: 'behavioral',
        details: {
          strategy: '24-hour impulse rule',
          items: [{ name: 'Shopping spend', amount: m.shopping, frequency: 'monthly' }],
          monthlyAmount: m.shopping,
          targetReduction: shopSave,
          reasoning: 'Your shopping spend of \u00a3' + m.shopping + '/mo is above average. ' + (impulsePattern ? impulsePattern.detail + ' ' : '') + 'Research shows that waiting 24 hours before purchases over \u00a330 eliminates about 20% of impulse buys. This saves \u00a3' + shopSave + '/mo (\u00a3' + (shopSave * 12) + '/yr) without any real sacrifice \u2014 you\u2019ll only skip things you didn\u2019t truly want.',
          steps: ['When you find something over \u00a330, add it to a wishlist instead of buying', 'Wait 24 hours before revisiting', 'If you still want it after 24 hours, buy it guilt-free', 'Track how many items you skip \u2014 most won\u2019t feel like losses']
        }
      });
    }
